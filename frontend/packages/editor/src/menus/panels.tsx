...
export const TURN_INTO_TARGETS: TurnIntoTarget[] = [
  ...
  { label: 'Heading 1', type: 'heading', props: { level: 1, isToggleable: false }, icon: <Heading1Icon size={16} /> },
  { label: 'Heading 2', type: 'heading', props: { level: 2, isToggleable: false }, icon: <Heading2Icon size={16} /> },
  { label: 'Heading 3', type: 'heading', props: { level: 3, isToggleable: false }, icon: <Heading3Icon size={16} /> },
  { label: 'Heading 4', type: 'heading', props: { level: 4, isToggleable: false }, icon: <Heading4Icon size={16} /> },
  { label: 'Toggle heading 1', type: 'heading', props: { level: 1, isToggleable: true }, icon: <ChevronRightSquareIcon size={16} /> },
...
function Overlay({ title, onClose, children, testId }: { title: string; onClose: () => void; children: ReactNode; testId: string }) {
  ...
  return (
    <div className="nook-overlay" role="dialog" aria-label={title} data-testid={testId} onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
...
export function TurnIntoPanel({ editor, blockIds, onClose }: { editor: AnyEditor; blockIds: string[]; onClose: () => void }) {
  return (
            ...
            onClick={() => {
              editor.transact(() => {
                for (const id of blockIds) {
                  ...
                  editor.updateBlock(id, { type: t.type, ...(t.props ? { props: t.props } : {}) } as never);
                }
              });
              onClose();
              editor.focus();
...
export function ColorPanel({ editor, blockIds, onClose }: { editor: AnyEditor; blockIds: string[]; onClose: () => void }) {
  ...
  return (
    <Overlay title="Colour" onClose={onClose} testId="color-panel">
      ...
      <div className="nook-overlay__colors">
        {['default', ...names].map((c) => (
          <button key={`t-${c}`} type="button" className="nook-overlay__swatch" data-kind="text" data-color={c} onClick={() => apply({ textColor: c })}>
            <span>A</span>
            {c}
          </button>
        ))}
...
export function PanelShell({ title, onClose, children, testId }: { title: string; onClose: () => void; children: ReactNode; testId: string }) {
  return (
