...
// and §7.4 quick find (tree.ts is still empty, so the mention / move-to pickers are served here —
...
export function createFilesHandlers(ctx: MockContext): HttpHandler[] {
  ...
  return [
    ...
    // ---- §7.4 quick find (temporary home: see the file header)
    http.get('/api/search/quick', ({ request }) => {
      ...
      return HttpResponse.json(
        quickFind(ctx.state, ws.id, q, limit)
          .filter((hit) => (kinds.length ? kinds.includes(hit.node.kind) : true))
          .map((hit) => ({
            node: hit.node,
            breadcrumb: hit.breadcrumb.map((n) => ({ id: n.id, title: n.title, icon: n.icon, kind: n.kind, parentId: n.parentId })),
            score: hit.score,
          })),
      );
    }),
