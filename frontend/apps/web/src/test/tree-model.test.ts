...
const mk = (id: string, parentId: string | null, position: string, extra: Partial<Node> = {}): Node => ({
  id,
  workspaceId: 'ws',
  parentId,
  kind: 'page',
...
const levels = new Map<string, Level>([
  ['root', { nodes: [mk('A', null, 'a0', { hasChildren: true }), mk('B', null, 'a1'), mk('C', null, 'a2', { hasChildren: true }), mk('Z', null, 'a3', { archivedAt: 'x' })], isPending: false }],
...
describe('flattenTree', () => {
  it('lists visible rows depth-first, hides archived unless asked', () => {
    ...
    expect(flattenTree(levels, { A: true, C: true }, false).find((i) => i.id === 'C')?.loading).toBe(true);
  });
...
describe('computeDrop', () => {
  ...
  it('bottom quarter = after; the horizontal offset picks the depth', () => {
    ...
    expect(computeDrop({ items, activeId: 'B', overId: 'A2', ratio: 0.9, offsetX: -30 })).toBeNull();
